...
def evaluate_candidate(
    ...
    if not hybrid["daily_bullish"]:
        reasons_against.append("daily_hybrid_hull_not_aligned")
    if not hybrid["weekly_bullish"]:
        reasons_against.append("weekly_hma21_hma51_not_aligned")
    if not hybrid["kama_rising"]:
        reasons_against.append("kama30_not_rising")
    if hybrid["stretched"]:
        reasons_against.append("extended_above_hybrid_hull")
    if hybrid["chop"]:
        reasons_against.append("hybrid_hull_chop")
    hard_block = (stale_data or regime.upper() == "BEAR" or not plan.valid
                  or not hybrid["daily_bullish"] or not hybrid["weekly_bullish"]
                  or not hybrid["kama_rising"] or hybrid["stretched"] or hybrid["chop"])
