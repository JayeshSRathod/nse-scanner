...
def _component_lines(candidate: Candidate) -> list[str]:
    row = candidate.horizon_scores.get(candidate.primary_horizon, {})
...
def _chunk_cards(cards: list[str], header: str, limit: int = 3800) -> list[str]:
    """Pack complete cards into Telegram-safe messages without splitting a card."""
    if not cards:
        return []
    messages: list[str] = []
    current = header
    ...
    if current:
        messages.append(current)
    total = len(messages)
    return [message.replace(header, f"{header} — {index}/{total}", 1) for index, message in enumerate(messages, start=1)]
...
def _action_card(
    ...
    if allocation:
        lines.append(
            f"Qty: {allocation.quantity} | Capital: {_price(allocation.entry_notional)} | Initial Risk: {_price(allocation.initial_risk)}"
        )
...
def render_candidate_messages(
    ...
    *,
    ...
    """Return static Telegram messages containing every actionable candidate."""
    ...
    summary = [
        ...
        f"Watchlist: {len(watch_rows)}",
    ]
    if not action_rows:
        summary.extend(["", "No ACTION candidate has both qualified quality and a READY trade plan today."])

    messages = ["\n".join(summary)]
    cards = [_action_card(candidate, rank, allocations) for rank, candidate in enumerate(action_rows, start=1)]
    messages.extend(_chunk_cards(cards, "🟢 ALL ACTIONABLE CANDIDATES"))

    if watch_rows:
        watch_lines = ["🟡 WATCHLIST — QUALIFIED, NO READY ENTRY"]
        for index, candidate in enumerate(watch_rows, start=1):
            watch_lines.append(
                f"{index}. {candidate.symbol} | {candidate.primary_horizon} {candidate.score:.0f} | {_watch_reason(candidate)}"
            )
        watch_text = "\n".join(watch_lines)
        if len(watch_text) <= 3900:
            messages.append(watch_text)
        else:
            rows = watch_lines[1:]
            messages.extend(_chunk_cards(rows, watch_lines[0]))
    return messages
...
def render_candidate_preview(
    ...
    *,
    ...
    """Backward-compatible single-message preview for tests and local inspection."""
    rows = [candidate for candidates in grouped.values() for candidate in candidates]
    messages = render_candidate_messages(
        rows[:max_candidates], [], regime, trade_date,
