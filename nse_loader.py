...
Tables:
    ...
    index_perf     -- Daily index performance (147 indices)
...
Usage:
    ...
    python nse_loader.py --all --cleanup         # Load + cleanup in one step
"""
...
# Files deleted after load (large, already in DB)
DELETE_AFTER_LOAD = [
...
# Minimum row counts to accept a parsed file
MIN_ROWS = {
    "bhavdata"  : 1000,
    "blacklist" : 10,
    "ind_close" : 100,
...
def validate(name, data, trade_date):
    ...
    needs = {"bhavdata":["symbol","close","date"],
    ...
    # Check close is not mostly null
...
def load_day(trade_date, do_cleanup=False):
    ...
    result     = {"date": trade_date, "status": "ok",
                  "rows": {}, "skipped": [], "errors": []}
...
def show_status():
    if not os.path.exists(DB_PATH):
        ...
        print(f"  Run: python nse_loader.py --all")
        return
    ...
    for table in ["daily_prices","blacklist","index_perf","volatility","week52","pe_ratios"]:
        try:
            ...
            print(f"  {table:<20} rows={r[0]:>7,}  days={r[1]:>3}  "
                  f"range={r[2] or 'n/a'} to {r[3] or 'n/a'}")
        except Exception:
            print(f"  {table:<20} -- not found")

    ll = conn.execute(
        "SELECT status, COUNT(*) FROM load_log GROUP BY status").fetchall()
    if ll:
...
def main():
    ...
    p.add_argument("--cleanup", action="store_true",
                   help="Delete large files after loading")
    ...
    if not days:
        ...
        print(f"  Run: python nse_historical_downloader.py --last 90")
        return
